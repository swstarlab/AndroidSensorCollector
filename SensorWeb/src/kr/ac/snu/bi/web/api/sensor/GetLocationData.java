...
public class GetLocationData extends WebAPI {
	...
	public String run(JSONObject query, HttpServletResponse response) throws Exception {
		...
		try {
			...
			resultSet = connection.query("SELECT COUNT(*) as count FROM Location WHERE time >= ? AND time <= ? AND userId=? ORDER BY time ASC", from, to, userId);
			if (resultSet.next()){
				int count = resultSet.getInt("count");
				if (count > COUNT_MAX){
					throw new Exception("GetLocationData: too many results");
				}
			}
			else{
				throw new Exception("GetLocationData: counting failed");
			}
			resultSet.close();
			
			resultSet = connection.query("SELECT * FROM Location WHERE time >= ? AND time <= ? AND userId=? ORDER BY time ASC", from, to, userId);
